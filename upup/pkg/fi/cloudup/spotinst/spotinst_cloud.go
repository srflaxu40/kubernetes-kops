...
type Cloud interface {
	...
	ListResources(filters []string) (map[string]*Resource, error)
...
func (c *cloud) ListResources(filters []string) (map[string]*Resource, error) {
	...
	case kops.CloudProviderAWS:
		{
			...
			for _, group := range out.Groups {
				...
				for _, filter := range filters {
					if strings.Contains(groupName, filter) {
						resource := &Resource{
						...
						glog.V(2).Infof("Discovered resource: %s (%s)", resourceKey, groupName)
					}
				}
			}
		}
